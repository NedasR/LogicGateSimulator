...
void AndGate::notifey()
{
	state = inputA.state && inputB.state;
	output.UpdateState(state);
}

void AndGate::drawGate(sf::RenderWindow& window)
{
...
void AndGate::moveupdate(sf::Vector2f pos)
{
	if (shape.isHeld(pos)) {
...
void AndGate::clickupdate(sf::Vector2f pos)
{
